...
// This is a bad example of abstraction. It uses only one mega-class
// as well as no get/set methongs for data-access and manipilation.
			...
			while (!done)
			{
				...
				if (choice == 1)
				{
					...
					if (array_1 == 1)
					{
						...
						{
							cout << (one[index_1] + two[index_2]) << endl;
						}
					}
					else
					{
						if (array_2 == 1)
						{
							cout << (two[index_1] + one[index_2]) << endl;
						}
						else
						{
							cout << (two[index_1] + two[index_2]) << endl;
						}
					}
